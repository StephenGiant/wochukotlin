...
private abstract static class AutoPlayHandler extends Handler{
    public AutoPlayHandler(){
...
//        super.handleMessage(msg);
    ...
    public abstract void doInHandleMessage(Message msg);
}
...
private AutoPlayHandler mAutoPlayHandler  = new AutoPlayHandler() {
    @Override
    public void doInHandleMessage(Message msg) {
        mCurrentPositon++;
        mViewPager.setCurrentItem(mCurrentPositon);
        mAutoPlayHandler.sendEmptyMessageDelayed(WHAT_AUTO_PLAY, mAutoPalyTime);
    }
...
//        mAutoPlayHandler.sendEmptyMessageDelayed(WHAT_AUTO_PLAY, mAutoPalyTime);
    ...
    public void clearImgs() {
        ...
        if(mImageUrls.size()>0) {
            for (int x = 0; x < mImageUrls.size(); x++) {
                if (x < imageViews.size()) {

                    Glide.clear(imageViews.get(x));
    ...
    private class FlyPageAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, final int position) {
            ...
            if(mImageUrls.size()>0) {
                if (mIsImageUrl) {
//                Glide.with(getContext()).load(mImageUrls.get(toRealPosition(position))).fitCenter().into(imageView);
//                GlideUtil.setImgFromNetAsbitmap(getContext(),mImageUrls.get(toRealPosition(position)),imageView);
                    //0.7比例缩放不影响图片质量且内存占用最小
                    GlideUtil.setImgFromNetWithSize(mContext, mImageUrls.get(toRealPosition(position)), imageView, (int) (FlyBanner.this.getMeasuredWidth() * 0.7f),
                            (int) (FlyBanner.this.getHeight() * 0.7f));
                } else {
